...
; R5 contains current sum
            ...
            .ORIG x3000		      ; ERROR FOUND: missing .ORIG directive

            LD    R1,INPUT            ; R1 contains input number
            LD    R2,R2VAL            ; R2 contains -1
            ADD   R3,R1,R2            ; R3 contains input number -1
            ADD   R3,R3,R2            ; ERROR FOUND: extra comma R3 contains input number -2
...
INNERLOOP   ADD   R5,R5,R1            ; Increment sum
            ...
            LD    R5,CLEAR            ; ERROR FOUND Clear R5 (previous sum) to 0
            ADD   R3,R3,R2            ; Decrement outer count
            BRzp  OUTERLOOP           ; ERROR FOUND: BRpz instead of zp Branch to outer loop if outer count
                                      ; is positive or zero

            ST   R1,DEST               ; This address contains X!          
            TRAP x25
	   
INPUT      .FILL  x0005               ; Input for X!, in this case X = 5 
CLEAR	   .FILL  x0000               ; Value 0
R2VAL      .FILL  xFFFF               ; 2's complement of 1 (i.e. -1)
DEST       .FILL  x3011               ; At program completion, the result is
                                      ; stored here
	   .END			      ; ERROR: no .END
